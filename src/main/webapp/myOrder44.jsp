...
<body>
	...
	<sql:query dataSource="${buydetails}" var="bd">
           		SELECT * FROM buydetails b join product p on b.p_id=p.p_id join shop s on p.s_id=s.s_id  where u_id='<%=session.getAttribute("id")%>' and bd_state='true' and bd_confirm="true" and bd_delivery='true' and bd_comment="false"
            </sql:query>
	...
	<c:forEach items="${bd.rows}" var="b">
		<table class="bordered">
			...
			<tr>
				...
				<td width="16%"><a href="comment?p_id=${b.p_id}&&bd_id=${b.bd_id}">评论</a></td>
			</tr>
		</table>
	</c:forEach>
</body>
